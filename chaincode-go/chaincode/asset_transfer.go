...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface) error {
	...
	if len(assetInput.Hash) == 0 {
		return fmt.Errorf("hash field must be a non-empty string")
	}
	if len(assetInput.Time) == 0 {
		return fmt.Errorf("time field must be a non-empty string")
	}
	if len(assetInput.Sbom) == 0 {
		return fmt.Errorf("sbom field must be a non-empty string")
	}
	...
	asset := Asset{
		...
		ID:    assetInput.ID,
		Hash:  assetInput.Hash,
		Time:  assetInput.Time,
		Owner: clientID,
	}
